...
const vec3 ROTATION_AXIS = vec3(0.05, 1, 0);
const float ROTATION_SPEED = 0.55;

const float MOON1_RADIUS = 0.06;
const vec3 MOON1_POS = vec3(11, 1, -2);

const float MOON2_RADIUS = 0.05;
const vec3 MOON2_POS = vec3(13, 3, -4);

const float MOON3_RADIUS = 0.07;
const vec3 MOON3_POS = vec3(10, -2, 3);

const vec3 LAND_COLOR = vec3(0.88, 0.82, 0.72);
const vec3 JUNGLE_COLOR = vec3(0.65, 0.48, 0.35);
const vec3 DESERT_COLOR = vec3(0.95, 0.90, 0.82);
const vec3 SNOW_COLOR = vec3(0.90, 0.86, 0.80);

const float OCEAN_SIZE = 0.52;
const vec3 OCEAN_COLOR = vec3(0.70, 0.58, 0.45);

const vec3 ATMOSPHERE_COLOR = vec3(0.85, 0.78, 0.65);
const float ATMOSPHERE_DENSITY = 0.8;
const vec3 DAWN_COLOR = vec3(0.90, 0.80, 0.65);
const vec3 SUNSET_COLOR = vec3(0.75, 0.55, 0.40);
const vec3 CLOUD_COLOR = vec3(0.95, 0.92, 0.85);

const float AMBIENT_LIGHT = 0.15;
const float SHADOW_STRENGTH = 0.6;

const vec3 LIGHT1_POS = vec3(8, 8, -8);
const vec4 LIGHT1_COLOR = vec4(1.0, 0.97, 0.92, 1.2);
const vec3 LIGHT2_POS = vec3(-6, 4, 6);
const vec4 LIGHT2_COLOR = vec4(0.8, 0.75, 0.65, 0.10);
...
float swirly_fbm(vec2 p, float scale, int octaves) {
    ...
    for (int i = 0; i < octaves; i++) {
        ...
        a *= 0.5;
        scale *= 2.2;
        p += vec2(cos(s * TAU), sin(s * TAU)) / scale * 0.6;
    }
    return s / m;
}
...
vec4 planet(vec2 uv) {
    ...
    if (TYPE == 1) {
        ...
        vec2 spot_center = vec2(0.25, 0.61);

        vec2 spot_delta = (raw_muv - spot_center) * vec2(8.0, 16.0);
        float spot_dist = length(spot_delta);

        float spot_mask = smoothstep(1.0, 0.4, spot_dist);
        vec3 spot_color = mix(
            vec3(0.72, 0.32, 0.18),
            vec3(0.82, 0.52, 0.35),
            smoothstep(0.0, 1.0, spot_dist)
        );
        float spot_swirl = fbm(raw_muv * 18.0 + uTime * 0.01, 1.0, 3) * 0.15;

        spot_mask *= (1.0 - spot_swirl);
        col = mix(col, spot_color, spot_mask * 0.85);
    }
...
vec4 atmosphere(vec2 uv) {
    ...
    vec3 col = mix(ATMOSPHERE_COLOR, sunset, exp(-altitude * 16.0) * smoothstep(PLANET_RADIUS - 0.01, PLANET_RADIUS, len) * sunset_amount * 1.5);
