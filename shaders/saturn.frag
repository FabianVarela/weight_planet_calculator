...
const float SEED = 19.4;

vec3 CAMERA = vec3(0, 0, -1);

const float PLANET_RADIUS = 0.75;

const vec3 ROTATION_AXIS = vec3(0.47, 1, 0);
const float ROTATION_SPEED = 0.45;

const float MOON1_RADIUS = 0.07;
const vec3 MOON1_POS = vec3(12, 4, -3);
const float MOON2_RADIUS = 0.04;
const vec3 MOON2_POS = vec3(14, -2, 4);
const float MOON3_RADIUS = 0.03;
const vec3 MOON3_POS = vec3(999, 999, 999);

const vec3 LAND_COLOR = vec3(0.92, 0.82, 0.58);
const vec3 JUNGLE_COLOR = vec3(0.75, 0.60, 0.35);
const vec3 DESERT_COLOR = vec3(0.98, 0.90, 0.68);
const vec3 SNOW_COLOR = vec3(0.88, 0.80, 0.62);

const float OCEAN_SIZE = 0.55;
const vec3 OCEAN_COLOR = vec3(0.80, 0.65, 0.40);

const vec3 ATMOSPHERE_COLOR = vec3(0.95, 0.82, 0.48);
const float ATMOSPHERE_DENSITY = 1.6;
const vec3 DAWN_COLOR = vec3(1.00, 0.85, 0.50);
const vec3 SUNSET_COLOR = vec3(0.88, 0.55, 0.20);
const vec3 CLOUD_COLOR = vec3(0.96, 0.90, 0.70);

const float AMBIENT_LIGHT = 0.20;
const float SHADOW_STRENGTH = 0.5;

const vec3 LIGHT1_POS = vec3(8, 8, -8);
const vec4 LIGHT1_COLOR = vec4(1.0, 0.95, 0.85, 1.3);
const vec3 LIGHT2_POS = vec3(-6, 4, 6);
const vec4 LIGHT2_COLOR = vec4(0.8, 0.65, 0.35, 0.25);

const int TYPE = 1;

const float RING_INNER = 1.15;
const float RING_OUTER = 2.10;
const vec3 RING_COLOR_A = vec3(0.82, 0.72, 0.55);
const vec3 RING_COLOR_B = vec3(0.65, 0.55, 0.38);
const vec3 RING_COLOR_C = vec3(0.90, 0.82, 0.65);
...
void main() {
    ...
    float len = length(uv);
    const float RING_SQUISH = 0.22;

    float ring_r = length(vec2(uv.x, uv.y / RING_SQUISH));
    float t = (ring_r - RING_INNER) / (RING_OUTER - RING_INNER);

    float ell_out = (uv.x * uv.x) / (RING_OUTER * RING_OUTER) + (uv.y * uv.y) / (RING_OUTER * RING_OUTER * RING_SQUISH * RING_SQUISH);
    float ell_in = (uv.x * uv.x) / (RING_INNER * RING_INNER) + (uv.y * uv.y) / (RING_INNER * RING_INNER * RING_SQUISH * RING_SQUISH);

    bool in_ring = (ell_out <= 1.0 && ell_in >= 1.0);

    vec4 ring_col = vec4(0.0);
    if (in_ring && t >= 0.0 && t <= 1.0) {
        float bn = noise(vec2(t * 40.0, 0.5), 1.0) * 0.3 + noise(vec2(t * 120.0, 0.5), 1.0) * 0.15;
        float density = 0.0;

        if (t < 0.05) density = smoothstep(0.0, 0.05, t) * 0.70;
        else if (t < 0.30) density = 0.72 + bn * 0.10;
        else if (t < 0.68) density = 0.95 + bn * 0.05;
        else if (t < 0.72) density = 0.15;
        else density = 0.80 + bn * 0.15;

        density = clamp(density, 0.0, 1.0) * smoothstep(1.00, 0.94, ell_out) * smoothstep(1.00, 1.12, ell_in);
        vec3 rc = vec3(0.0);

        if (t < 0.30) rc = RING_COLOR_B;
        else if (t < 0.68) rc = mix(RING_COLOR_A, RING_COLOR_B, bn * 0.5);
        else if (t < 0.72) rc = RING_COLOR_B * 0.5; // Cassini — color visible
        else rc = RING_COLOR_C;

        rc *= LIGHT1_COLOR.rgb * 0.95;
        ring_col = vec4(rc, density * 0.98);
    }

    float planet_shadow_on_rings = smoothstep(1.08, 0.92, len) * smoothstep(-0.02, 0.08, uv.y);
 ...
 * smoothstep(0.0, 0.5, - (dist - distance(mp, lp)) / mr); }
    ...
    vec3 glow_col = ATMOSPHERE_COLOR * LIGHT1_COLOR.rgb * LIGHT1_COLOR.a * glow * 0.4;

    float behind = smoothstep(-0.02, 0.04, uv.y);
    float front = 1.0 - behind;

    fragColor = vec4(0.0, 0.0, 0.0, 1.0);
    if (!on_planet) fragColor.rgb += glow_col;

    if (in_ring) fragColor.rgb = mix(fragColor.rgb, ring_col.rgb, ring_col.a * behind);

    if (on_planet) fragColor.rgb = mix(fragColor.rgb, planetCol.rgb, planetCol.a);

    if (in_ring) fragColor.rgb = mix(fragColor.rgb, ring_col.rgb, ring_col.a * front);
