...
const float SEED = 8.1;

vec3 CAMERA = vec3(0, 0, -1);

const float PLANET_RADIUS = 0.75;

const vec3 ROTATION_AXIS = vec3(0.28, 1, 0);
const float ROTATION_SPEED = 0.30;

const float MOON1_RADIUS = 0.05;
const vec3 MOON1_POS = vec3(11, 3, -2);
...
const vec3 LAND_COLOR = vec3(0.10, 0.30, 0.80);
const vec3 JUNGLE_COLOR = vec3(0.08, 0.20, 0.65);
const vec3 DESERT_COLOR = vec3(0.20, 0.45, 0.90);
const vec3 SNOW_COLOR = vec3(0.35, 0.60, 0.95);

const float OCEAN_SIZE = 0.55;
const vec3 OCEAN_COLOR = vec3(0.05, 0.15, 0.60);

const vec3 ATMOSPHERE_COLOR = vec3(0.15, 0.40, 0.95);
const float ATMOSPHERE_DENSITY = 2.2;
const vec3 DAWN_COLOR = vec3(0.20, 0.50, 1.00);
const vec3 SUNSET_COLOR = vec3(0.10, 0.25, 0.80);
const vec3 CLOUD_COLOR = vec3(0.60, 0.75, 1.00);

const float AMBIENT_LIGHT = 0.18;
const float SHADOW_STRENGTH = 0.3;

const vec3 LIGHT1_POS = vec3(8, 8, -8);
const vec4 LIGHT1_COLOR = vec4(0.80, 0.88, 1.00, 1.10);
const vec3 LIGHT2_POS = vec3(-6, 4, 6);
const vec4 LIGHT2_COLOR = vec4(0.20, 0.40, 0.90, 0.15);
...
vec4 planet(vec2 uv) {
    ...
    if (TYPE == 1) {
        ...
        // Gran Mancha Oscura
        vec2 raw_muv = mercator(norm);
        vec2 spot_center = vec2(0.30, 0.45);
        vec2 spot_delta = (raw_muv - spot_center) * vec2(7.0, 14.0);
        float spot_dist = length(spot_delta);
        float spot_mask = smoothstep(1.0, 0.3, spot_dist);
        float spot_swirl = fbm(raw_muv * 16.0 + uTime * 0.015, 1.0, 3) * 0.12;
        spot_mask *= (1.0 - spot_swirl);
        vec3 spot_color = mix(
            vec3(0.02, 0.08, 0.40),
            vec3(0.08, 0.20, 0.65),
            smoothstep(0.0, 1.0, spot_dist)
        );
        col = mix(col, spot_color, spot_mask * 0.80);
    }
...
vec4 clouds(vec2 uv) {
    ...
    vec3 norm = get_norm(uv, z);
    uv = mercator(norm);
    float c = swirly_fbm(-uv, 11.0, 6) * smoothstep(1.0, 0.99, abs(norm.y));
    c = exp(-pow(c, 6.) * 32.0);
    return vec4(CLOUD_COLOR, c);
}
...
vec4 atmosphere(vec2 uv) {
    ...
    sunset_amount = min(sunset_amount,
                        smoothstep(0.5, -0.3, dot(norm, normalize(LIGHT2_POS))));
    float density = exp(-altitude * 7.0 * (1.0 + sunset_amount));
    density *= l2 * ATMOSPHERE_DENSITY;
    vec3 sunset = mix(DAWN_COLOR, SUNSET_COLOR,
                      exp(-altitude * 32.0) * sunset_amount);
    vec3 col = mix(ATMOSPHERE_COLOR, sunset,
                   exp(-altitude * 16.0)
                   * smoothstep(PLANET_RADIUS - 0.01, PLANET_RADIUS, len)
                   * sunset_amount * 1.5);
 ...
 * smoothstep(0.0, 0.5, - (dist - distance(mp, lp)) / mr); }
    ...
    * LIGHT2_COLOR.rgb * LIGHT2_COLOR.a;
    ...
    vec4 atm_col = atmosphere(uv);
    p.rgb *= shadow * light + AMBIENT_LIGHT;
    atm_col.rgb *= light;
    fragColor = mix(p, atm_col, atm_col.a);
